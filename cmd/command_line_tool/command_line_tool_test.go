...
// getDefaultConfigs returns all standard settings
// for the server, the logger and the CLI.
func getDefaultConfigs() (serverConfig structs.ServerConfig, logConfig structs.LogConfig, cliConfig structs.CLIConfig) {
...
func cleanupTestFiles(config structs.ServerConfig) {
	if filehandler.DirectoryExists(config.Tickets) {
		if removeErr := os.RemoveAll(config.Tickets); removeErr != nil {
			testlogger.Debug("test error: could not remove ticket directory:", removeErr)
		}
	}

	if filehandler.DirectoryExists(config.Mails) {
		if removeErr := os.RemoveAll(config.Mails); removeErr != nil {
			testlogger.Debug("test error: could not remove mail directory:", removeErr)
		}
	}
}
...
func TestGetConfig(t *testing.T) {
	...
	_, _, cliConfig := getDefaultConfigs()
	cliConfig.Port = 8443
...
func startTestServer(t *testing.T, serverConfig structs.ServerConfig) (shutdown func()) {
	...
	shutdown = func() {
		...
		testlogger.Debug(<-startError)
	}
	return
}

// Test the loop in commandLoop() with all available commands
func TestCommandLoop(t *testing.T) {
	...
	serverConfig, _, cliConfig := getDefaultConfigs()
	...
	t.Run("submitCommandValidInputs", func(t *testing.T) {
		...
		// Type submit command
		commandReader.appendCommand(structs.SUBMIT)
		...
		commandReader.appendCommand(structs.EXIT)

		testlogger.Debug("Using following user inputs:", commandReader.String())
	...
	t.Run("fetchCommand", func(t *testing.T) {
		...
		commandReader.appendCommand(structs.FETCH)
...
func TestMainFunction(t *testing.T) {
	...
	serverConfig, _, _ := getDefaultConfigs()
	...
	t.Run("noFlagsSet", func(t *testing.T) {
		...
		t.Run("noFatalErrors", func(t *testing.T) {
			assert.True(t, fatalErrors.Len() == 0, "The fatal error buffer should be empty")
		})
	})
	...
	t.Run("noFlagsSetError", func(t *testing.T) {
		...
		*cert = defaults.TestCertificate
		...
		testlogger.Debug("Using following flags:")
		testlogger.Debug("  -cert", *cert)
	...
	t.Run("submitFlagSet", func(t *testing.T) {
		...
		*cert = defaults.TestCertificate
		...
		testlogger.Debug("Using following flags:")
		testlogger.Debug("  -s", *submit)
		testlogger.Debug("  -email", *email)
		testlogger.Debug("  -subject", *subject)
	...
	t.Run("submitAndFetchFlagSet", func(t *testing.T) {
		...
		*fetch = true
		...
		t.Run("fatalErrorMessageIsOutsideRangeOptions", func(t *testing.T) {
			assert.Contains(t, fatalErrors.String(), "Not within the range of valid options",

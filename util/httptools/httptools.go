...
// jsonContentType is used as a constant content
// type for json responses.
const jsonContentType string = "application/json; charset=utf-8"

// contentTypeOptions is the default value for
// the "X-Content-Type-Options" header used in
// error responses.
const contentTypeOptions string = "nosniff"

// StatusCodeError writes an error message given by
// the cause parameter alongside with a HTTP status
// code and its description to the given response
// writer. The status code should be an erroneous
// one as the function implies it.
...
// JSONResponse writes a json response to the supplied
// response writer by converting the map containing json
// properties to a valid json string.
...
// JSONError writes a json error response with the
// properties contained in jsonProperties and sets
// the response status code which should be an
// erroneous one.
...
// responseToJSON converts the json properties map
// to a json string and returns it.

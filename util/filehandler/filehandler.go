...
// ReadUserFile takes a string as parameter for the
// location of the users.json file, reads the content
// and stores it inside of the hash map for the users.
func ReadUserFile(srcFile string, users *map[string]structs.User) error {

	// Read contents of users.json
	fileContent, errReadFile := ioutil.ReadFile(srcFile)
	...
	if errUnmarshal != nil {
		return wrapAndLogErrorf(errUnmarshal, "unable to decode JSON in users file '%s'", srcFile)
	}

	return nil
}

// WriteUserFile writes the contents of the users map
// to the file system to persist any changes.
func WriteUserFile(destFile string, users *map[string]structs.User) error {
	...
	// Write json to file
	return ioutil.WriteFile(destFile, usersMarshal, defaults.FileModeRegular)
}

// ReadTicketFiles reads all the tickets into memory at
// the server start.
func ReadTicketFiles(directory string, tickets *map[string]structs.Ticket) error {
	...
	for _, f := range files {
		...
		if hasJSONExtension(f.Name()) {
			...
			fileContent, errReadFile := ioutil.ReadFile(directory + "/" + f.Name())
			...
			if errUnmarshal != nil {
				return wrapAndLogErrorf(errUnmarshal, "could not decode JSON in ticket file '%s/%s'", directory, f.Name())
			}
...
// WriteTicketFile writes a given ticket to a given
// directory in the json format. If the ticket already
// exists, it overwrites its contents.
func WriteTicketFile(directory string, ticket *structs.Ticket) error {

	// Check if directory exists. If not, create it
	if !DirectoryExists(directory) {

		log.Info("Creating missing ticket directory", directory, "for new ticket file")
		errCreateFolders := CreateFolders(directory)
		if errCreateFolders != nil {
			log.Error(errCreateFolders)
	...
	log.Info("Writing ticket file", finalPath, "to file system (Permission = 0644 [rw-r--r--])")
	return ioutil.WriteFile(finalPath, marshalTicket, defaults.FileModeRegular)
}

// ReadMailFiles lookups the files in the given directory,
// reads them and decodes JSON files into mail structures.
// Those structures are added to a mail hash map with its
// id as key.
...
// WriteMailFile takes a mail and converts it into the
// JSON format to write it into its own file. The directory
// parameter is a path to a directory in which the new file
// is saved. If it does not exist yet it will be created.
func WriteMailFile(directory string, mail *structs.Mail) error {

	// If the directory does not exist yet, create it
	if !DirectoryExists(directory) {

		log.Info("Creating missing mail directory", directory, "for new mail file")
	...
	log.Info("Writing mail file", mailFilePath, "to file system (Permission = 0644 [rw-r--r--])")
...
// RemoveMailFile attempts to remove a mail with a given id
// in a given directory. If the file does not exist, it
...
func RemoveMailFile(directory string, mailID string) error {
	...
	if removeErr := os.Remove(mailPath); removeErr != nil {
		...
		log.Errorf("%v: %v", returnErr, removeErr)
		return returnErr
	}
...
// FileExists examines if a given file path exists and is a
// regular file or not.
// Inspired by https://gist.github.com/mattes/d13e273314c3b3ade33f
func FileExists(filePath string) bool {
	if file, notExistsErr := os.Stat(filePath); !os.IsNotExist(notExistsErr) {
...
// DirectoryExists reports whether the given directory path
// exists and points to a directory or not.
// Inspired by https://stackoverflow.com/a/49697453
func DirectoryExists(dirPath string) bool {
	if file, notExistsErr := os.Stat(dirPath); !os.IsNotExist(notExistsErr) {
...
// hasJSONExtension checks if the given filename argument
// conforms to the glob pattern "*.json", i.e. if the the
// filename has a .json file extension.
...
// wrapAndLogError creates a new error by wrapping an error
// message around an existing error. Before it returns the
// error, the function logs its message to the console.
...
// wrapAndLogErrorf creates a new error by wrapping an error
// message around an existing error. The error message is
// constructed using a printf like format string and the
// corresponding arguments to the placeholders. Before it
// returns the error, the function logs its message to the
// console.

...
// logger is the default logger which writes its output
...
// logln writes a newline delimited log message consisting of the
// arguments in v with the specified log level to the standard log.
func logln(level structs.LogLevel, v ...interface{}) {
	logger.Println(level.String(), joinWithoutNewline(v...), getLoggingLocationSuffix(4))
}

// logf writes a formatted message with the specified log level
// to the standard log.
func logf(level structs.LogLevel, format string, v ...interface{}) {
	logger.Println(level.String(), fmt.Sprintf(format, v...), getLoggingLocationSuffix(4))
}
...
var fatalln FatalFunc = logger.Fatalln

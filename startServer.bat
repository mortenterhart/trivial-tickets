...
set program_name=trivial-tickets
set root_dir=%~dp0
set main_executable=cmd\ticketsystem\ticketsystem.go
...
REM  Check if the ticketsystem main executable exists
IF EXIST "%root_dir%\%main_executable%" (
    ...
    IF %ERRORLEVEL% EQU 0 (
        ...
        REM  Execute the ticketsystem executable
        call :info "Starting Ticketsystem web server"
...
REM  as first argument $1 and should be enclosed by
REM  double quotes.
:error
    IF "%program_name%" == "" set program_name=trivial-tickets

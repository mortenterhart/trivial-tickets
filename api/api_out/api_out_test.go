...
// runTests executes all the tests that were triggered by a call to
// 'go test'. It ensures that the deferred call of the mail cleanup
// function is executed before the test binary is quit with `os.Exit()`.
...
func cleanupMails() {
	...
	for id := range globals.Mails {
		testlog.Debugf("Deferred: Removing mail '%s' from mail hash map", id)
	...
	if filehandler.DirectoryExists(globals.ServerConfig.Mails) {
		testlog.Debug("Deferred: Removing test mail directory", globals.ServerConfig.Mails)
		if removeErr := os.RemoveAll(globals.ServerConfig.Mails); removeErr != nil {
			testlog.Debug("ERROR: could not remove test mail directory:", removeErr)
		}
	}
}
...
func TestFetchMails(t *testing.T) {
	...
	t.Run("invalidPOSTRequest", func(t *testing.T) {
		const jsonRequest string = `{"id":"test-id"}`
...
func TestVerifyMailSentInvalidJSON(t *testing.T) {
	...
	const invalidJSON string = `{id:"mail-id"}`
...
func TestVerifyMailSentMissingRequiredProperties(t *testing.T) {
	...
	const missingProperties string = `{"email":"admin@example.com"}`
...
func TestVerifyMailSentAdditionalProperties(t *testing.T) {
	...
	const additionalProperties string = `{"id":"mail-id","email":"admin@example.com"}`
...
func TestVerifyMailSentInvalidPropertyTypes(t *testing.T) {
	...
	const propertyTypes string = `{"id":200}`

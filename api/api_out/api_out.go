...
// and constructs a new mail which is then saved into
// its own file. The message of the mail is wrapped
// inside a mail template depending on the event.
func SendMail(mailEvent mail_events.Event, ticket structs.Ticket) {
	...
	log.Infof(`Composing notification mail (id "%s") to '%s' for %s`,
		newMail.ID, newMail.To, mailEvent.String())

	globals.Mails[newMail.ID] = newMail

	log.Info("Saving new mail as", globals.ServerConfig.Mails+"/"+newMail.ID+".json")
...
// FetchMails is an endpoint to the outgoing Mail
// API and sends all mails which are currently cached
// and ready to be sent. The response is in JSON format.
//
//     Takes: no parameters
//     Returns: {
//         "<mail_id>": {
...
func FetchMails(writer http.ResponseWriter, request *http.Request) {
	...
	if request.Method == "GET" {
		...
		log.Infof("%d %s: Delivering %d mail(s) as response to client", http.StatusOK,
	...
	log.Errorf("%d %s: request sent with wrong method '%s', expecting 'GET'", http.StatusMethodNotAllowed,
		http.StatusText(http.StatusMethodNotAllowed), request.Method)
}

// idParameter is the required id parameter for the
// VerifyMailSent handler. It has to be passed as
// JSON formatted string and requires the string data
// type.
const idParameter string = "id"

// VerifyMailSent can be used by an external service to
// verify that a mail was sent. It requests an unique mail
// id and checks if the corresponding mail exists inside
// the cache. If it does, the mail can be safely deleted
// and the API returns a verified JSON object. If the mail
// does not exist, the API returns an unverified object.
func VerifyMailSent(writer http.ResponseWriter, request *http.Request) {
	...
	if request.Method == "POST" {
		...
		if _, mailExists := globals.Mails[mailID]; !mailExists {
			...
			log.Infof("%d %s: Verification of mail '%s' failed: mail does not exist or has already been deleted",
				http.StatusOK, http.StatusText(http.StatusOK), mailID)
			return
		}

		log.Infof("Removing mail '%s' from global mail storage", mailID)
		delete(globals.Mails, mailID)

		log.Info("Deleting mail file", globals.ServerConfig.Mails+"/"+mailID+".json")
		...
		log.Infof("%d %s: Verified sending of mail '%s' successfully and deleted from server cache",
			http.StatusOK, http.StatusText(http.StatusOK), mailID)
		return
	}
...
// verifyMailCheckRequiredPropertiesSet takes JSON properties
// and checks if the required property "id" is set.
...
// verifyMailCheckAdditionalProperties checks if any other
// properties than "id" are set within the JSON request.
...
// verifyMailCheckPropertyTypes examines the type of the
// value of the property "id" and verifies its correctness.
...
// convertToJSON converts a json map into a json string
